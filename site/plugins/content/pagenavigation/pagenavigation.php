...
class PlgContentPagenavigation extends JPlugin
{
	...
	public function onContentBeforeDisplay($context, &$row, &$params, $page = 0)
	{
      ...
      // Multicats override
      //echo JRequest::getVar('catid');
      if(JRequest::getVar('catid')){ $catid = JRequest::getVar('catid');}
      elseif(isset($row)) { $catid = $row->catid; }

      $catarray = explode(',',$catid);
      $catids = explode(':',$catarray[0]);
      $cid = $catids[0];
      $q = "SELECT alias FROM #__categories WHERE id = ".$cid;
      $db->setQuery($q);
      $alias = $db->loadResult();
      //End Multicats
      ...
      $row->catid = JRequest::getVar('catid'); // Multicats
      $query->select('a.id, a.title, a.catid, a.language,' . $case_when . ',' . $case_when1)
				->from('#__content AS a')
				->join('LEFT', '#__categories AS cc ON cc.id = a.catid')
      // Multicats - multicats override
				->where('FIND_IN_SET( '. (int)$row->catid .', a.catid )  AND a.state = '. (int)$row->state .' AND a.state = ' . (int) $row->state
      ...
      //end Multicats        
			...
			if ($row->prev)
			{
				...
				$row->prev = JRoute::_(ContentHelperRoute::getArticleRoute($row->prev->slug, JRequest::getVar('catid'), $row->prev->language)); // multicats
        //$row->prev = JRoute::_(ContentHelperRoute::getArticleRoute($row->prev->slug, $row->prev->catid, $row->prev->language));
			...
			if ($row->next)
			{
				...
				$row->next = JRoute::_(ContentHelperRoute::getArticleRoute($row->next->slug, JRequest::getVar('catid'), $row->next->language)); // multicats
