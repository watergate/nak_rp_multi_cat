...
class ContentModelArticles extends JModelList
{
    ...
    //Multicats m.ordering for a.ordering
		$orderCol = $app->input->get('filter_order', 'm.ordering');
	...
	protected function getListQuery()
	{
      ...
      */
      ...
      // Add subcategory check
			$includeSubcategories = $this->getState('filter.subcategories', false);
      # changed for multicats
 			$categoryEquals = '((a.catid  LIKE "'.$categoryId.'")  OR (a.catid  LIKE "'.$categoryId.',%") OR (a.catid  LIKE "%,'.$categoryId.',%") OR (a.catid  LIKE "%,'.$categoryId.'")) ';
      /* End Multicats */    
        ...
        //Mcats
        $squery = $subQuery->__toString();
        $db->setQuery($squery);
        $sids = $db->loadObjectList();
        $subwhere = array();
        foreach($sids as $sid){		
          $subwhere[] = ' FIND_IN_SET('.$sid->id.',a.catid) ';  
        }
        $subwhere_list = '';
        if(count($subwhere) > 0) {
          $subwhere_list = '('. implode(' OR ',$subwhere) .')';
        }          
        
        if($categoryEquals != '' AND $subwhere_list != '') { $categoryEquals = $categoryEquals . ' OR '; }
        $query->where('('.$categoryEquals . $subwhere_list.')');
        //end mcats                  
			}
			else
			{
    ...
    // Multicats Join over the multicats ordering
		...
		$cat_id = ((int)$categoryId > 1) ? (int)$categoryId : 'a.catid';   
    $query->join('LEFT', '#__content_multicats AS m ON (m.content_id = a.id AND m.catid='.$cat_id.')'); 
    // End Multicats
