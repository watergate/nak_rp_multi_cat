...
class ContentModelArticle extends JModelItem
{
	...
	public function getItem($pk = null)
	{
        ...
        // Technically guest could edit an article, but lets not check that to improve performance a little.
				if (!$user->get('guest'))
				{
					// Multicats - check all categories - v3.4.6.4
          $catids = explode(',',$data->catid);
          $restricted = false;
          foreach($catids as $catid){
            if(!$user->authorise('core.edit', 'com_content.category.'.$catid)){
          ...
          if ($user->authorise('core.edit', $asset) && !$restricted) // Multicats - restricted 
					{
