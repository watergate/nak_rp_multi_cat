...
class ContentModelFeatured extends ContentModelArticles
{
	...
	protected function getListQuery()
	{
		...
		{
			JArrayHelper::toInteger($featuredCategories);
			$featuredCategories = implode(',', $featuredCategories);
			if (!empty($featuredCategories)) {
			 	$type = $this->getState('filter.category_id.include', true) ? 'IN' : 'NOT IN';
				$featuredCategories = explode(',', $featuredCategories);
				$cond = array();
				for($x=0;$x<count($featuredCategories);$x++){
				 
				 	$cid = $featuredCategories[$x];
					$cond[] = 'FIND_IN_SET("'.$cid.'",a.catid)';
				} 
				if($type == 'IN')
					$query->where( implode(' OR ',$cond ));
				  else
				  	$query->where('a.catid '.$type.' ('.$featuredCategories.')');
